...
type QueueTicket = { id: string; clinicName: string; queueNumber: number; status: "waiting" | "called" | "served" | "missed" | "left"; scheduledAt: string; calledAt: string | null; callExpiresAt: string | null; position: number | null; peopleAhead?: number; estimatedWait: number; patients: number; wait: number; requestedMedication?: string | null; medicationCollected?: boolean; collectedAt?: string | null };
...
function PatientQueue({ clinic }: { clinic: Clinic }) {
  ...
  const joinQueue = async () => {
    ...
    setMessage("");
    try {
      const payload = selectedMedication ? { medication: selectedMedication } : {};
  ...
  const statusText = ticket?.status === "called" ? ticket?.requestedMedication ? `Please proceed to collect ${ticket.requestedMedication}.` : "Please proceed to the clinic desk now." : ticket?.status === "served" ? ticket?.medicationCollected ? `${ticket.requestedMedication ?? "Medication"} has been handed over.` : "Your visit is complete." : ticket?.status === "missed" ? "This number was missed. Request a new number to join again." : ticket?.status === "left" ? "You left this queue." : "Your number is active.";
  return <section className="detail-card queue-checkin-card"><div className="section-heading"><div><p className="eyebrow">PATIENT CHECK-IN</p><h2>Request a queue number</h2><p>Each clinic has its own live queue. Your allocated time updates from the people ahead of you.</p></div>{ticket && <StatusPill value={ticket.status === "waiting" ? "Waiting" : ticket.status} />}</div>{ticket ? <><div className="ticket-number">#{displayedQueueNumber}<small>{ticket.status === "left" ? "YOUR ISSUED QUEUE NUMBER" : "YOUR QUEUE NUMBER"}</small></div>{ticket.requestedMedication && <div className="queue-message">Medication: <strong>{ticket.requestedMedication}</strong>{ticket.medicationCollected ? " • Collected" : " • Pending collection"}</div>}<div className="queue-ticket-details"><span><strong>{peopleAhead}</strong> people ahead</span><span><strong>{ticket.estimatedWait}</strong> min estimated wait</span><span><strong>{displayedScheduledAt ? new Date(displayedScheduledAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : "-"}</strong> allocated time</span></div><p className="queue-message">{statusText}</p>{ticket.status === "waiting" && <button className="secondary-button" disabled={loading} onClick={leaveQueue}>{loading ? "Updating..." : "Leave queue"}</button>}{["missed", "served", "left"].includes(ticket.status) && <button className="primary-button" disabled={loading || clinic.status === "Closed"} onClick={() => { setTicket(null); setToken(""); setMessage(""); }}>{clinic.status === "Closed" ? "Clinic closed" : "Request a new number"}</button>}</> : <><div className="queue-ticket-details"><span><strong>{clinic.patients}</strong> patients ahead</span><span><strong>{clinic.patients * getWaitPerPatient(clinic.status)}</strong> min estimated wait</span><span><strong>{clinic.status === "Closed" ? "Closed" : "Next available"}</strong> service</span></div><p>See how many patients are ahead before requesting your queue number.</p>{availableMedications.length > 0 && <label className="queue-label">Medication for collection<select value={selectedMedication} onChange={(event) => setSelectedMedication(event.target.value)}>{availableMedications.map((medication) => <option key={medication.name} value={medication.name}>{medication.name}</option>)}</select></label>}<button className="primary-button" disabled={loading || clinic.status === "Closed" || availableMedications.length === 0} onClick={joinQueue}>{loading ? "Requesting..." : clinic.status === "Closed" ? "Clinic currently closed" : availableMedications.length === 0 ? "No medication in stock" : `Get my queue number for ${selectedMedication}`}</button>{message && <p className="queue-error">{message}</p>}</>}</section>;
}
...
function MedicationCollectionTickets({ clinicName }: { clinicName: string }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (active) setTickets((data.tickets ?? []).filter((ticket) => Boolean(ticket.requestedMedication)));
      } catch {
  ...
  return <section className="portal-panel"><div className="section-heading"><div><h2>Medication collection queue</h2><p>Tickets requesting medication handover for this clinic.</p></div></div>{loading ? <p className="empty">Loading medication collection tickets…</p> : tickets.length === 0 ? <p className="empty">No medication collection tickets are in progress.</p> : <div className="staff-crud-list">{tickets.map((ticket) => <div className="staff-crud-row" key={ticket.id}><span><strong>#{ticket.queueNumber}</strong><small>{ticket.requestedMedication}</small></span><span className="staff-actions"><StatusPill value={ticket.medicationCollected ? "Collected" : ticket.status === "called" ? "Ready to collect" : ticket.status === "served" ? "Served" : "Pending"} /></span></div>)}</div>}</section>;
}
